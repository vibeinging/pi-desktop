// agent 设置页:整窗接管(自带左栏 + 主面板),与对话壳同主题(--agent-* token)。
...
type NetworkSettings = Pick<AgentSettingsData, 'httpProxy' | 'noProxy' | 'customCert'>
...
function pickNetworkSettings(settings: AgentSettingsData): NetworkSettings {
  return {
...
function saveDesktopNetworkSettings(settings: AgentSettingsData) {
...
export function applyAgentZoom(zoom: AgentSettingsData['zoom']) {
  // 缩放只作用于 .agent-zoom(内容)。外层 .agent-root 的 padding / 标题栏条不被缩放,保持固定。
  const el = document.querySelector('.agent-zoom') as HTMLElement | null
  if (!el) return
  const f = ZOOM_FACTOR[zoom] ?? 1
  // 不使用 CSS zoom:它会和 Electron/Chromium page zoom 混在一起,导致右侧/底部露出背景。
  el.style.removeProperty('zoom')
  el.style.setProperty('--agent-zoom', String(f))
}
...
const GENERAL_GROUP_NAV: NavDef[] = [
  ...
  { key: 'privacy', label: '隐私', Icon: IconShieldCheck, desc: '本机诊断与体验优化偏好。' }
]
...
const THEME_LABEL: Record<AgentThemeMode, string> = {
  light: '浅色',
  dark: '暗色',
  system: '跟随系统'
}
      ...
      <div className={`${styles.mainInner} ${active === 'skills' || active === 'mcp' ? styles.mainInnerFixed : ''}`}>
          {isGeneralPage ? (
            <>
              ...
              <SettingsSection title="网络" desc="配置模型、MCP 和命令工具的网络出口。">
                <TextRow
                  label="HTTP 代理"
                  desc="模型、MCP 与命令工具的出口流量将经此代理;留空时直连。保存后渲染层立即生效,后端请求需重启应用。"
              ...
              {(isGeneralOverview || active === 'privacy') && (
              <SettingsSection title="隐私" desc="调整本机诊断与体验优化偏好。">
