...
import type { UiModuleSummary } from '@/api/uiModules'
...
export default function YiWNav({
  ...
  return (
    <>
      ...
      {modules.length > 0 && (
        <>
          <div className={styles.secLabel}>功能</div>
          <div className={styles.moduleList}>
            {modules
              .filter((item) => item.status === 'active' && item.sidebar?.visible !== false)
              .sort((a, b) => Number(a.sidebar?.order || 100) - Number(b.sidebar?.order || 100))
              .map((item) => (
                <button
                  key={item.id}
                  type="button"
                  className={`${styles.moduleItem} ${item.id === activeModuleId ? styles.moduleItemActive : ''}`}
                  onClick={() => onSelectModule?.(item.id)}
                  title={item.description || item.name}
                >
                  {String(item.icon || '').includes('chart') ? <IconChartLine size={15} stroke={1.8} /> : <IconSparkles size={15} stroke={1.8} />}
                  <span>{item.name}</span>
                  {item.current_version?.version && <small>v{item.current_version.version}</small>}
                </button>
              ))}
          </div>
        </>
